...
function init() {
	...
	camera = new THREE.PerspectiveCamera(50, window.innerWidth / window.innerHeight/2, 1, 6000);
	
	camera.position.set(76, 204, 688);
//	camera.focus = 20;
	var gui = new dat.GUI();
	gui.add( params, 'focus', 1, 200 ).onChange( function (value) {
		camera.focus = Number(value);
	//	camera.update();
	} );
	gui.open();
...
// 	effectPZ = new THREE.ParallaxBarrierEffect( renderer );
// 	effectPZ.setSize( window.innerWidth, window.innerHeight*2  );
